...
const imageModules = import.meta.glob(
  "../../content/posts/**/*.{jpg,jpeg,png,webp,svg}",
  {
    eager: true,
    import: "default",
  },
...
export function postDirectory(post: BlogPost) {
  const sourceDirectory = post.filePath?.match(
    /(?:^|\/)posts\/(.+)\/index\.mdx$/,
  )?.[1];
  if (sourceDirectory) {
...
function resolvePostAsset(post: BlogPost, assetPath?: string) {
  ...
  return imageModules[
    `../../content/posts/${postDirectory(post)}/${cleanPath}`
  ];
}
...
export function getAllTags(posts: BlogPostView[]) {
  return [...new Set(posts.flatMap(({ post }) => post.data.tags ?? []))].sort(
    (a, b) => slugify(a).localeCompare(slugify(b)),
  );
}
